...
export async function generateReadMe(): Promise<void> {
  const bundleFileContent: string = await readFile(Path.Dist.Library.main, { encoding: 'utf-8' });
  const bundleFileSizeKB: number = Array.from(Buffer.from(bundleFileContent, 'utf-8')).length / 1024;

  const originalContent: string = await readFile(Path.Source.Library.readMe, { encoding: 'utf-8' });
  ...
  return writeFile(Path.Dist.Library.readMe, resultContent, {
    encoding: 'utf-8',
  });
}
